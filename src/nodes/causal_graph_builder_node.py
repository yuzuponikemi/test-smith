...
def causal_graph_builder_node(state: dict) -> dict:
    ...
    print(
        f"  Building causal graph with {len(hypotheses)} hypotheses and {len(symptoms)} symptoms..."
    )

    # Create lookup for ranked likelihood
    likelihood_map = {h["hypothesis_id"]: h["likelihood"] for h in ranked_hypotheses}
    ...
    return {"causal_graph_data": graph_data}
