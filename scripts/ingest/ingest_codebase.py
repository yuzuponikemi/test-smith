...
DEFAULT_SKIP_DIRS = {
    ...
    'node_modules', 'vendor', '.venv', 'venv', '__pycache__', '.pytest_cache',  # Dependencies/cache
    'dist', 'build', 'target', 'out', 'bin',  # Build outputs
    '.idea', '.vscode', '.eclipse',  # IDE configs
    'coverage', '.nyc_output', 'htmlcov',  # Coverage reports
    '.tox', '.eggs', '*.egg-info',  # Python packaging
}
        ...
        for pattern in self.patterns:
            ...
            if pattern.endswith('/'):
                ...
                if fnmatch.fnmatch(rel_path, dir_pattern) or \
                   fnmatch.fnmatch(rel_path, f"*/{dir_pattern}") or \
                   rel_path.startswith(dir_pattern + '/'):
                    return True
            ...
            try:
                ...
                doc = Document(
                    ...
                    metadata={
                        ...
                        **{k: v for k, v in analysis.metadata.items()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Ingest a codebase into ChromaDB for coding agent'
    )
