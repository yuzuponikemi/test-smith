...
class PreprocessedIngestion:
    ...
    def print_final_report(self, failed_files: List[str], metrics: Dict):
        ...
        reduction_rate = (
            (self.stats['total_chunks_before_cleaning'] - self.stats['total_chunks_after_cleaning'])
            / self.stats['total_chunks_before_cleaning']
        ) if self.stats['total_chunks_before_cleaning'] > 0 else 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Ingest documents with intelligent preprocessing'
    )
    ...
    parser.add_argument(
        '--skip-analysis',
        action='store_true',
        help='Skip document analysis phase'
    )

    parser.add_argument(
        '--disable-deduplication',
        action='store_true',
        help='Disable near-duplicate detection'
    )
