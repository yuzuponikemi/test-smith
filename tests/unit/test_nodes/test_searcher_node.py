...
class TestSearcherNode:
    ...
    def test_successful_single_search(self, mock_manager_class, mock_print_header):
        ...
        # Arrange
        state = {"web_queries": ["What is machine learning?"]}
        ...
        mock_manager.search.assert_called_once_with("What is machine learning?", max_results=5, attempt_all=True)
    ...
    def test_successful_multiple_searches(self, mock_manager_class, mock_print_header):
        ...
        assert result["search_results"][0] == [{"content": "Python is...", "url": "https://example.com/python"}]
        assert result["search_results"][1] == [{"content": "JavaScript...", "url": "https://example.com/js"}]
        assert result["search_results"][2] == [{"content": "Docker is...", "url": "https://example.com/docker"}]
    ...
    def test_individual_search_failure(self, mock_manager_class, mock_print_header):
        ...
        # Arrange
        state = {"web_queries": ["Query that will fail"]}

        # Mock manager instance
        mock_manager = MagicMock()
        mock_manager.get_available_providers.return_value = ["tavily", "duckduckgo"]
        mock_manager.search.side_effect = Exception("All search providers failed (tavily, duckduckgo). Last error: API rate limit")
        mock_manager_class.return_value = mock_manager
    ...
    def test_mixed_success_and_failure(self, mock_manager_class, mock_print_header):
        ...
        assert result["search_results"][0] == [{"content": "Success 1", "url": "https://example.com/1"}]
        ...
        assert result["search_results"][2] == [{"content": "Success 2", "url": "https://example.com/2"}]
    ...
    def test_empty_search_results(self, mock_manager_class, mock_print_header):
        ...
        # Arrange
        state = {"web_queries": ["Very obscure query"]}
